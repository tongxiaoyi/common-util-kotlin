...
object FileUtil {
    ...
    fun zip(outputZipFullFilename: String, directory: String, fileSuffix: String, zipProcessor: ZipProcessor) {
        val matchOption = MatchOption(directory)
        matchOption.fileSuffix = fileSuffix
        val fileList = FileUtil.findMatchFile(matchOption)
        if (!fileList.isEmpty()) {
    ...
    fun findMatchFileDirectory(matchOption: MatchOption): List<String> {
    ...
    fun findMatchFile(matchOption: MatchOption): List<String> {
    ...
    fun findMatchDirectory(matchOption: MatchOption): List<String> {
    ...
    private fun findMatchFileOrMatchFileDirectory(matchOption: MatchOption): List<String> {
        val list = ArrayList<String>()
        val queue = ConcurrentLinkedQueue<File>()
        val directoryFile = File(matchOption.directory)
        queue.add(directoryFile)
        while (!queue.isEmpty()) {
            val file = queue.poll()
            if (!file.exists()) {
            ...
            if (file.isDirectory) {
                ...
                if (matchOption.findType === MatchOption.FindType.FILE) {
                    val fullFilename = matchOption.processor?.onMatch(file) ?: file.absolutePath
                    // ignore when null
                    list.add(fullFilename)
                } else if (matchOption.findType === MatchOption.FindType.FILE_DIRECTORY) {
                    val parentFullFilename = matchOption.processor?.onMatch(file.parentFile)
     ...
     * match option
     */
    class MatchOption(internal val directory: String) {
